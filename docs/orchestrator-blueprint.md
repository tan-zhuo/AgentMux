...
> 状态：已确认。Phase 1、2、3 已实现，Phase 4 未开始。五个开放问题的结论见 §12
> 日期：2026-08-17
...
实现中定下来的六件事，都是写到一半才发现原计划没说清楚的：

1. **`internal/orch/catalog` 拆成独立包。** Skill 校验要查工具是否存在，引擎要用 skill
   做匹配——两边互相 import 就成环了。目录是共享词汇，机器是引擎，分开之后谁都不依赖谁。
2. **等人的时间不计入 15 分钟墙钟。** 这个限制是为了抓住跑飞的循环，不是为了惩罚一个
   花两分钟读审批卡的人。真按原样算，一次审批就能让整个 run 超时——那只会教人闭眼点同意。
3. **所有工具结果一律包成不可信数据块**，不只是"看起来来自远端"的那些。逐个工具判断
   哪个输出可信，是一道每加一个工具就要重做一次的判断题，错一次就够了。代价是几行上下文。
4. **同时只跑一个 run。** 两个循环对着同一批机器互相打架，各自的判断都建立在对方正在
   推翻的画面上，而且审批队列会变得没人知道自己在回答哪一次。
5. **启动时做一次恢复。** run 只存在于运行中的进程里，所以数据库里还写着 running 的
   都是上次进程死掉时留下的。不清理的话，界面上会永远挂着一个没人在做的任务。
6. **加了 `Explain`（干跑）。** 不执行任何东西，只回答"这个调用如果被提出来会怎样"。
   这是 v0.3 §6 说的"Skill 执行沙箱预览"，落在闸门这一层比落在 Skill 上更有用——它回答的
   是"这台机器到底有多信任"，而那正是人容易记错的东西。
