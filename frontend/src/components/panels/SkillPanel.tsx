...
import { t as tr, useFmt, useT } from '../../store/useI18n'
...
const statusKey: Record<SkillStatus, MsgKey> = {
    ...
    <div className="flex h-full flex-col">
      <div className="flex items-center justify-between border-b hairline px-3 py-2">
        <span className="text-[11px] font-semibold text-ink-300">
          {t('panel.skills')} {stats ? `(${stats.total})` : ''}
        </span>
        <div className="flex gap-1">
          <Button
            ...
            title={t('skill.testBench')}
          >
            <FlaskConical size={11} />
          </Button>
          <Button size="sm" variant="subtle" onClick={() => void importBundle(toast, load)} title={t('skill.import')}>
            <Upload size={11} />
          </Button>
          <Button size="sm" variant="subtle" onClick={() => void exportBundle(toast)} title={t('skill.export')}>
            <Download size={11} />
          </Button>
          <Button
            size="sm"
          ...
          <Button size="sm" variant="subtle" onClick={() => void load()} disabled={loading} title={t('common.refresh')}>
      ...
      <div className="border-b hairline px-3 py-2">
        <Segmented<Tab>
          ...
          onChange={setTab}
          options={[
            { value: 'active', label: t('skill.tab.active') },
            {
              value: 'draft',
              label: (
                <>
            ...
            { value: 'other', label: t('skill.tab.retired') },
          ]}
        />
      </div>
        ...
        <div className="flex items-start gap-2 border-b hairline bg-warn/5 px-3 py-2">
          ...
          <div className="min-w-0 flex-1">
            <p className="text-[11px] leading-relaxed text-ink-200">
              {stats.pending === 1
                ? t('skill.pendingOne', { model: stats.model })
                : t('skill.pending', { n: stats.pending, model: stats.model })}
            </p>
            <Button
              size="sm"
              variant="primary"
      ...
      <div className="min-h-0 flex-1 overflow-y-auto">
        {rows.length === 0 && !loading && (
          <Empty
            title={tab === 'draft' ? t('skill.noDrafts') : t('skill.none')}
            hint={tab === 'draft' ? t('skill.noDrafts.hint') : t('skill.none.hint')}
          />
        )}
        {rows.map((s) => (
  ...
  return (
    <div className="border-b hairline px-3 py-2">
      <div className="flex items-center gap-1.5">
        ...
        <Badge tone={statusTone[skill.status]}>{t(statusKey[skill.status] ?? 'skill.status.active')}</Badge>
        {skill.createdBy === 'orchestrator' && <Badge tone="accent">{t('skill.proposed')}</Badge>}
        {skill.status === 'active' && !skill.hasVector && (
          <Badge tone="warn">{t('skill.noVector')}</Badge>
        )}
      ...
      {open && (
        <div className="mt-2 space-y-2">
          ...
          <p className="text-[10px] text-ink-600">
            {skill.scope}
            {skill.usageCount > 0 && ` · ${t('skill.used', { n: skill.usageCount })}`}
            {skill.confidence !== null &&
              ` · ${t('skill.confidence', { score: skill.confidence.toFixed(2) })}`}
            {` · ${t('skill.updated', { date: fmt.date(skill.updatedAt) })}`}
          </p>
            ...
            {skill.status === 'draft' && (
              <>
                <Button
                  size="sm"
                  variant="primary"
                  onClick={() => void onAct(skill.id, 'approve', t('skill.approved'))}
                >
                  <Check size={11} /> {t('skill.approve')}
                </Button>
                <Button size="sm" onClick={() => void onAct(skill.id, 'reject', t('skill.rejected'))}>
                  <X size={11} /> {t('skill.reject')}
                </Button>
              </>
            )}
            {skill.status === 'active' && (
              <Button size="sm" onClick={() => void onAct(skill.id, 'disable', t('skill.disabled'))}>
                <Pause size={11} /> {t('skill.disable')}
              </Button>
            )}
            {skill.status === 'disabled' && (
              <Button size="sm" onClick={() => void onAct(skill.id, 'enable', t('skill.enabled'))}>
                <Play size={11} /> {t('skill.enable')}
              </Button>
            )}
            {skill.status === 'archived' && (
              <Button size="sm" onClick={() => void onAct(skill.id, 'restore', t('skill.restored'))}>
                <Play size={11} /> {t('skill.restore')}
              </Button>
            )}
            {(skill.status === 'active' || skill.status === 'disabled') && (
              <Button size="sm" onClick={() => void onAct(skill.id, 'archive', t('skill.archived'))}>
                <Archive size={11} /> {t('skill.archive')}
              </Button>
            )}
            <Button size="sm" onClick={() => openDialog({ kind: 'skill', skill })}>
              <Pencil size={11} /> {t('skill.edit')}
            </Button>
            ...
            <Button
              ...
              onClick={async () => {
                const ok = await confirmAction({
                  title: t('skill.delete.title', { name: skill.name }),
                  message: t('skill.delete.message'),
                  points: [t('skill.delete.point')],
  ...
  return (
    <div className="border-b hairline px-3 py-2">
      <textarea
        ...
        rows={2}
        placeholder={t('skill.scenarioPlaceholder')}
      ...
      <div className="mt-1.5 flex items-center gap-2">
        ...
        <span className="text-[10px] text-ink-600">{t('skill.matchOnly')}</span>
      </div>

      {matches !== null && (
        <div className="mt-2 space-y-1.5">
          {matches.length === 0 && (
            <p className="text-[11px] text-ink-500">{t('skill.noMatch')}</p>
          )}
          {matches.map((m) => (
...
async function importBundle(
  ...
  try {
    ...
    if (!text.trim()) {
      toast('error', tr('skill.clipboardEmpty'))
      return
    }
    ...
    if (res.skipped.length > 0) {
      ...
      toast(
        'ok',
        res.imported === 1 ? tr('skill.importedOne') : tr('skill.imported', { n: res.imported }),
      )
    }
    await reload()
  } catch (e) {
